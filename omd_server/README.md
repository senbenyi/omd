...
| 手机号 | 密码 | 说明 |
|--------|------|------|
| `13800138000` | `abc123456` | 用户 Id 1（王老板），拥有川味小馆（涩谷店）、樱寿司（新宿店） |
| `13900139000` | `abc123456` | 用户 Id 2（李老板），拥有老王牛肉面、粤式茶餐厅（池袋店）、韩式烤肉 |
| `13800000000` | `123456` | 用户 Id 3（马老板） |

`mock/stores.json` 中每家店铺的 `userId` 字段表示该店 `ownerUserId`，启动时会同步到数据库。
