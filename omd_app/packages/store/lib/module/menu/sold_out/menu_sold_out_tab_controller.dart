...
/// 已售罄 Tab 独立状态与接口。
class MenuSoldOutTabController extends GetxController {
  ...
  Future<bool> restoreStock({
    ...
    try {
      final response = await MenuApi.updateItemSoldOut(
        storeId,
        UpdateMenuItemSoldOutRequest(itemId: itemId, soldOut: false),
      );
      if (!response.isSuccess) {
        showAppToast(
          response.message.isNotEmpty
