...
abstract final class MenuOffShelfLayout {
  static const divider = Color(0xFFE4E7EC);
}

class MenuOffShelfEmptyState extends StatelessWidget {
  const MenuOffShelfEmptyState({
    super.key,
...
class MenuOffShelfPanelHeader extends StatelessWidget {
  const MenuOffShelfPanelHeader({
    super.key,
    required this.title,
    this.subtitle,
    this.leadingIcon,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: const BoxDecoration(
        color: Colors.white,
        border: Border(bottom: BorderSide(color: MenuOffShelfLayout.divider)),
      ),
      child: Row(
        children: [
          if (leadingIcon != null) ...[
